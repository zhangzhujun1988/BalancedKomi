...
//设置搜索时对局手数的最大值 
const int MAX_PLAY_NUMBER = 25;		

//设置白方被提子的数量的最大值 
const int CAPTURE_WHITE_LIMIT = 2;	

//设置初始着法 
const int INIT_PLAY_NUMBER = 1;		//初始着法数量 
...
public:
	...
	//辅助值成员 
	Node* father;			//表示父节点 
	long long hashValue;	//表示局面的 hash 值，用于判断局面是否同形 
	int captureBlackNumber;	//表示黑子被提掉的数量 
	int captureWhiteNumber;	//表示白子被提掉的数量 
...
int Node::WhiteAlive() {
	...
	countTreeHeight[height] += 1;
	
	//检查手数是否已经超过头文件定义的最大手数，如果已经超过则直接放弃该分支 
	if (height + 1 > MAX_PLAY_NUMBER) {
		return 	NOTALIVE;
	} 
	
	//用于调试 
	if (height <= PRINT_TREE_HEIGHT) {
